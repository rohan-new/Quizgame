...
function onRemovePlayer (data) {
    ...
    socket.emit('winbyquit',{id: socket.id});
        socket.emit('remove_fromlist_win_movies',{
        id:socket.id,
        room__id: room_id
    });
        ...
        function answer(ans){
            ...
            socket.on('blink_enemies',(data)=>{
             ...
             $('#score3').removeClass('no-animation');
            });
           ...
           socket.on('winner',(data)=>{
            ...
            socket.on('lose',(data)=>{
                ...
                socket.emit('remove_fromlist_lose_movies',{
                    id:socket.id,
                    room__id : room_id
                });
            });
            socket.on('draw',(data)=>{
                ...
                socket.emit('remove_fromlist_draw_movies',{
                    id:socket.id,
                    room__id : room_id
                });
            });
  ...
  // socket connection
    socket.on('connect',function(){
      ...
      socket.emit('new_player_movies',{
          id:socket.id 
      });
    });
    
    // game
    socket.on('movies',gameplay);
    socket.on('movies_1',gameplay1);

      //listen to new enemy connections
      socket.on("new_enemyPlayer_movies", onNewPlayer);
